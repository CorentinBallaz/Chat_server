...
//on affecte à la liste locale les users contenu dans redis


io.on('connection', function (socket) {
  client.lrange("usersDB",0,-1,function(err,reply){
    console.log(reply);
    users = reply;
  });
  /**
  ...
  socket.on('disconnect', function () {
    ...
    if (loggedUser !== undefined) {
      ...
      client.lrem(['usersDB',0,loggedUser.username], function(err, reply) { 
	    });
	    client.lrange("usersDB",0,-1, function(err,reply) { 
        console.log(reply);
		    users=reply;
  ...
  socket.on('user-login', function (user, callback) {
    ...
    if (user !== undefined && userIndex === -1) { // S'il est bien nouveau
      ...
      client.rpush(['usersDB', loggedUser.username], function(err, reply) {
	  });
